...
public class ClientImpl implements Client {
	...
	public ClientImpl(TunnelTestGUI gui)
	{
		m_gui = gui;
	}
	...
	public void receiveMessages()
	{
		while(true)
		{
			...
			{
				byte[] input = read(in);
				m_gui.printf("Receiving encrypted bytes: " + DataTypeConversionHelpers.ByteToString(input));
	...
	public boolean sendMessage(String s) 
	{
		try
		{
			...
			m_gui.printf("Bytes: " + DataTypeConversionHelpers.ByteToString(output));
	...
	public boolean startClient(String host, int port, String sharedKey) 
	{
		...
		try
		{
			...
			DiffieHellmanMethodHelper helpme = new DiffieHellmanMethodHelper();
			...
			m_gui.printf("Waiting for ServerImpl respond with its challenge...");
...
//			inFromServer.read(serverPublicKeyEncoded);
			...
			m_gui.printf("Challenge From ServerImpl: " + new String(serverChallenge));
			...
			if (!clientChallengeFromServer.equals(clientChallenge)) {
				m_gui.printf("ServerImpl returned incorrect challenge, terminating program");
				return false;
			}
