...
import com.shky.network.vpn.core.tunnel.NetworkTunnel;
import com.shky.network.vpn.core.encryption.DiffieHellmanMethodHelper;
import com.shky.network.vpn.test.gui.TunnelTestGUI;
import com.shky.network.vpn.core.helpers.DataTypeConversionHelpers;

public class ServerImpl implements NetworkTunnel {
	...
	public ServerImpl(TunnelTestGUI gui)
	{
		m_gui = gui;
	}
	...
	public void receiveMessages()
	{
		while(true)
		{
			try
			{
				...
				m_gui.printf("Receiving encrypted bytes: " + DataTypeConversionHelpers.ByteToString(clientInput));
	...
	public boolean sendMessage(String s) 
	{
		try
		{
			...
			m_gui.printf("Bytes: " + DataTypeConversionHelpers.ByteToString(bytes) );
			byte[] encrypted = communicationEncryptionCipher.doFinal(bytes);
			m_gui.printf("Sending Encrypted bytes: " + DataTypeConversionHelpers.ByteToString(encrypted) );
	...
	public boolean startServer(int port, String sharedKey) {
		...
		{
			m_gui.printf("ServerImpl Error: " + e.getMessage() );
			return false;
		}
	...
	public void acceptConnection()
	{
		...
		try
		{
			...
			DiffieHellmanMethodHelper diffieHellmanMethodHelper = new DiffieHellmanMethodHelper();
			...
			m_gui.printf("ClientImpl challenge" + clientChallenge);
			
			// Generate Diffie-Hellman keypair, and send data to client to continue handshake. 
			AlgorithmParameterSpec diffieHellmanParameters = diffieHellmanMethodHelper.generateEncryptionMethodParameters();
	        m_gui.printf("Generating Diffie Hellman key pair for server");
	        KeyPair serverKeyPair = diffieHellmanMethodHelper.createKeyPair(diffieHellmanParameters);
	        ...
	        PublicKey clientPublicKey = diffieHellmanMethodHelper.getUnencodedPublicKey(unencryptedEncodedPublicKeyBytes);
			diffieHellmanMethodHelper.getKeyAgreement().doPhase(clientPublicKey, true);
			
			byte[] serverSharedKey = diffieHellmanMethodHelper.getKeyAgreement().generateSecret();
			...
			diffieHellmanMethodHelper.getKeyAgreement().doPhase(clientPublicKey, true);
			SecretKey serverDESKey = diffieHellmanMethodHelper.getKeyAgreement().generateSecret("DES");
